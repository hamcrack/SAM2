        ...
        for i in range(len(masks)):
            ...
            if min_plate_area < seg_area < max_plate_area:
                ...
                if min_w_h_ratio < ratio < max_w_h_ratio and occupied > min_occupied_percentage:
                    ...
                    display_plate = cv2.rectangle(potential_plate, (x_bound, y_bound), (len(potential_plate[0])-x_bound, len(potential_plate)-y_bound), (255, 0, 0), 2)
                    ...
                    for j in range(no_sub_masks):
                        ...
                        avg_color_per_row = np.average(potential_plate, axis=0)
                        avg_color = np.average(avg_color_per_row, axis=0)
                        hsv_img = cv2.cvtColor(np.array([[avg_color, avg_color], [avg_color, avg_color]], dtype=np.uint8), cv2.COLOR_BGR2HSV)
                        its_blue = False
                        print("         ave color: ", avg_color, ", hue:", hsv_img[0][0])
                        if 100 < int(hsv_img[0][0][0]) < 110 and 126 < int(hsv_img[0][0][1]):
                            print("         its blue")
                    ...
                    if no_valid_sub_masks > 3:
                        ...
                        print(f"    plate slope: {slope}, intercept: {intercept}, r_value: {r_value}, p_value: {p_value}, std_err: {std_err}")
                    ...
                    display_plate = cv2.circle(display_plate, (ave_valid_x, ave_valid_y), 3, (255, 0, 0), 2) 
                    if no_valid_sub_masks > 3 and ave_cent_dist < y_bound*2 and std_err < max_std_err:
                        print("    Found ", no_valid_sub_masks, " valid sub masks")
                        color = (0, 255, 0)
                        cv2.imshow('potential_plate', display_plate)
                        cv2.waitKey(0)
